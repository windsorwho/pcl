...
class IVFPQIndex(CellContainer):
    def __init__(
        ...
        self.n_probe = 1
        if pq_use_residual and (n_cells * 256 * n_subvectors *
                                4) <= 4 * 1024**3:
        ...
        self.vq_codec = VQCodec(n_clusters=n_cells,
                                n_redo=1,
                                max_iter=15,
                                tol=1e-4,
                                distance="euclidean",
        ...
        self.pq_codec = PQCodec(d_vector=d_vector,
    ...
    def use_precomputed(self, value):
        ...
        if value:
            ...
            assert (self.pq_codec.is_trained
                    and self.vq_codec.is_trained), "index is not trained"
            required_memory = (self.n_cells * self.n_subvectors *
                               1024) / 1024**2
            self.print_message(
                f"estimated memory for precomputed table is {required_memory} MB",
                1)
    ...
    def precompute_part2(self):
        ...
        vq_codebook = self.vq_codec.codebook.reshape(self.n_subvectors,
                                                     self.d_subvector,
                                                     self.n_cells)
        self._precomputed_part2 = torch.bmm(vq_codebook.transpose(
            -1, -2), pq_codebook) * -2 - pq_codebook.norm(dim=1).pow(2)[:,
                                                                        None]
    ...
    def precomputed_adc_residual_precomputed(self, x):
        ...
        x = x.reshape(self.n_subvectors, self.d_subvector,
                      n_query).transpose(-1, -2)
    ...
    def precomputed_adc_residual(self, x, cells):
        ...
        x = x.reshape(self.n_subvectors, self.d_subvector,
                      n_query).transpose(-1, -2)
        part1 = 2 * (x @ pq_codebook).permute(1, 0, 2) - pq_codebook.norm(
            dim=1).pow(2)[None]  #[n_query, n_sub, 256]

        vq_codebook = vq_codebook.reshape(self.n_subvectors, self.d_subvector,
                                          self.n_cells)
        vq_codebook = vq_codebook[:, :,
                                  cells]  #[n_sub, d_sub, n_query, n_probe]
        vq_codebook = vq_codebook.permute(0, 2, 3,
                                          1)  #[n_sub, n_query, n_probe, d_sub]
        pq_codebook = pq_codebook[:, None].expand(
            -1, n_query, -1, -1)  #[n_sub, n_query, d_sub, 256]
        part2 = -2 * (vq_codebook @ pq_codebook
                      )  #[n_sub, n_query, n_probe, 256]
        precomputed = part1[:, None] + part2.permute(
            1, 2, 0, 3)  #[n_query, n_probe, n_sub, 256]
        return precomputed
    ...
    def search(self, x, k=1, return_address=False):
        ...
        if self.use_smart_probing and self.n_probe > 1:
            ...
            normalized_entropy = -torch.sum(
                p * torch.log2(p) / torch.log2(max_n_probe), dim=-1)
            n_probe_list = torch.ceil(normalized_entropy * max_n_probe).long()
        else:
            n_probe_list = torch.zeros(
                n_query, dtype=torch.long, device=self.device) + self.n_probe
        util.tick("smart probing")

        result = self.search_cells(x=x,
                                   cells=cells,
                                   base_sims=topk_sims,
                                   n_probe_list=n_probe_list,
                                   k=k,
